...
function removeDuplicates(movies) {
  ...
  const resultado = removeDuplicates(movies);
  console.log(resultado);

/* Iteración #2: Mix Fors
Usa for of y for in para hacer la media del volumen de todos los 
...
let calcularMedia = function(user){
    let total = 0;
    let count = 0;
    
    for (const userData of user) {

    for (const soundType in userData["favoritesSounds"]) {
        total += userData["favoritesSounds"][soundType].volume;
        count++;
    }
    ...
    console.log(calcularMedia(userData));
